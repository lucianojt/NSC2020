...
if(isset($_POST["answer"])){
 ...
 foreach ($_POST["answer"] as $value) {
    ...
    //  echo  'hi!! '.$value."<br>" ; 
    $sql = "SELECT * FROM exercise_ans WHERE name = 'detailreserve' AND id= '".$value."'  ";
...
$sql3 = "SELECT * FROM scoreExercise 
        WHERE code = '$code' 
        AND user_usr = '$use' 
        AND score_detailreserve >= 0 ";
$result3 = mysqli_query($connection,$sql3);
$row3 = mysqli_fetch_assoc($result3);

if(!$row3 ){
  //เพิ่มคะแนนคนที่เล่นครั้งแรก scoreExercise เก็บเฉพาะครั้งแรก
  $sql2 = "UPDATE scoreExercise SET score_detailreserve = '$score' WHERE user_usr = '$use' AND code = '$code' ";
  $result2 = mysqli_query($connection,$sql2);
 
   
  //เพิ่มคะแนนครั้งแรกใน scoreDetailreserve เพิ่อเก็บจำนวนครั้งที่เล่น
  $sql5 = "INSERT INTO scoreDetailreserve VALUES(NULL,'$use','$code',$score)";
  $result5 = mysqli_query($connection,$sql5);

  //echo 'เล่นครั้งแรก';
}else{
  //เพิ่มคะแนนคนที่เล่นครั้งที่ 2,3,4,5,....... เพิ่มใน scoreDetailreserve
  $sql4 = "INSERT INTO scoreDetailreserve VALUES(NULL,'$use','$code',$score)";
...
?>
<br><br>
<a class="btn btn-primary" href="detail.php" role="button">กลับ</a>
</div>
